...
const dataService = {
  ...
  fetchRecentDocs: async (limit = 5) => {
    try {
      ...
      const queryParams = `sort=updatedAt:desc&pagination[limit]=${limit}&filters[$or][0][owner][documentId]=${userData.documentId}&filters[$or][1][isPublic][$eq]=true&populate[0]=file&populate[1]=owner`;
  ...
  fetchDashboardStats: async () => {
    try {
      ...
      const docsResponse = await api.get(`/api/docs/count?filters[$or][0][owner][id]=${userId}&filters[$or][1][isPublic][$eq]=true`);
