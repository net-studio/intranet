...
export const fetchEvents = async (startDate = null, endDate = null) => {
  try {
    ...
    let queryParams = `filters[$or][0][participants][user][id]=${userId}&filters[$or][1][isPublic][$eq]=true&sort=startDate:asc&populate[0]=organizer&populate[1]=participants.user`;
...
export const fetchEventById = async (id) => {
  try {
    const response = await api.get(`/api/events/${id}?populate[0]=organizer&populate[1]=location&populate[2]=participants.user&populate[3]=attachments`);
