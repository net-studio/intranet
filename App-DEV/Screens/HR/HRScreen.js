...
const HRScreen = () => {
  ...
  const loadHRData = async () => {
    try {
      ...
      const [slipsResponse, requestsResponse, policiesResponse, profileResponse] = await Promise.all([
        hrService.getSalarySlips(),
        hrService.getLeaveRequests(),
        hrService.getHRPolicies(),
        hrService.getEmployeeProfile(),
      ]);
    ...
    <View style={styles.hrOfficeSection}>
      <View style={styles.hrOfficeImageContainer}>
        ...
        <View style={styles.hprBadge}>
          <Text style={styles.hprText}>HPR</Text>
        </View>
      </View>
    </View>
  );
        ...
        <View>
          ...
          <Text style={styles.sectionSubtitle}>
            {leaveRequests.filter(req => req.status === 'pending').length > 0
              ? `${leaveRequests.filter(req => req.status === 'pending').length} en attente`
  ...
  const renderBottomButtons = () => (
    <View style={styles.bottomButtons}>
      ...
      >
        <Text style={styles.primaryButtonText}>Rémunération</Text>
      </TouchableOpacity>
